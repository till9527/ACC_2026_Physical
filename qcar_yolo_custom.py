...
while running YOLOv8 object detection natively in the main thread.
"""
import os
import signal
import numpy as np
...
if __name__ == "__main__":
    ...
    # 1. Initialize Camera Stream FIRST
    ...
    # Convert to TensorRT if it hasn't been done yet (Crucial for Jetson FPS)
    if not os.path.exists(engine_path):
        print(f"[INFO] {engine_path} not found. Exporting from {pt_path}...")
        print("[INFO] This will take a few minutes, but only happens once!")
        temp_model = YOLO(pt_path)
        temp_model.export(
            format="engine", imgsz=(imageHeight, imageWidth), dynamic=False, half=True
        )
        print("[INFO] Export complete!")

    # Load the optimized engine (or fallback to .pt if it failed)
    load_path = engine_path if os.path.exists(engine_path) else pt_path
    ...
    try:
        ...
        while controlThread.is_alive() and (not KILL_THREAD):
            ...
            if QCarImg.read():
                ...
                # verbose=False stops it from spamming the terminal each frame
                ...
                # Render the bounding boxes directly onto the image
                annotatedImg = results[0].plot()

                # Calculate and draw FPS
                end = time.time()
                computationTime = end - start
                fps = 1.0 / (computationTime + 0.0001)
                cv2.putText(
                    annotatedImg,
                    f"FPS: {fps:.1f}",
                    (10, 30),
                ...
                # Show Image
                cv2.imshow("Native Object Detection", annotatedImg)
